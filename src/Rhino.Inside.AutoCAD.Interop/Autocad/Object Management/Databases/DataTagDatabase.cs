...
/// <inheritdoc cref="IDataTagDatabase"/>
public class DataTagDatabase : IDataTagDatabase
{
    private readonly Dictionary<string, IDataTagRecord> _tagRecords;

    /// <inheritdoc/>
    public IDbObject DbObjectOwner { get; }

    /// <summary>
    /// Constructs a new <see cref="DataTagDatabase"/>.
    /// </summary>
    ...
    /// returned.
    /// </summary>
    private Dictionary<string, IDataTagRecord> GetExistingRecords(DBObject dbObject)
    {
        ...
        var dataRecords = new Dictionary<string, IDataTagRecord>();
        if (extensionDictionaryId != CADObjectId.Null)
        {
            ...
            foreach (var dictionaryEntry in extensionDictionary)
            {
                ...
                if (dictionaryObject is not Xrecord xRecord) continue;

                using var resultBuffer = xRecord.Data;

                if (resultBuffer == null) continue;

                var dataTagRecord = new DataTagRecord(key);
    ...
    public IDataTagRecord GetRecord(string key)
    {
        ...
        dataTagRecord = new DataTagRecord(key);
    ...
    public IEnumerator<IDataTagRecord> GetEnumerator()
    {
