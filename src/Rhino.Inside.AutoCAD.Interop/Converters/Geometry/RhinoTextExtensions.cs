...
/// <summary>
/// Provides extension methods for converting Rhino text types to AutoCAD text types.
/// </summary>
public static class RhinoTextExtensions
{
    ...
    /// <returns>An AutoCAD MText with coordinates and dimensions scaled to AutoCAD units.</returns>
    public static MText ToAutocadMText(this TextEntity rhinoText)
    {
        var mtext = new MText();
        ...
        mtext.TextHeight = UnitConverter.ToAutoCadLength(rhinoText.TextHeight * rhinoText.DimensionScale);

        if (rhinoText.FormatWidth > 0)
            mtext.Width = UnitConverter.ToAutoCadLength(rhinoText.FormatWidth * rhinoText.DimensionScale);

        mtext.Location = rhinoText.Plane.Origin.ToAutocadPoint3d();

        mtext.Normal = rhinoText.Plane.ZAxis.ToAutocadVector3d();
    ...
    /// </summary>
    /// <param name="justification">The Rhino text justification.</param>
    /// <returns>The corresponding AutoCAD attachment point.</returns>
    private static AttachmentPoint ConvertJustification(TextJustification justification)
    {
    ...
    private static string ConvertRichTextToMText(TextEntity rhinoText)
    {
        ...
        while (index < rtfContent.Length)
        {
            ...
            if (c == '\\' && index + 1 < rtfContent.Length)
            {
                ...
                var controlWord = ParseRtfControlWord(rtfContent, ref index);
    ...
    private static string ParseRtfControlWord(string content, ref int index)
    {
        // Skip the backslash
        index++;
